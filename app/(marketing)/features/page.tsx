...
export default function FeaturesPage() {
  return (
    <>
      <MarketingHeader currentPage="features" showSearch={true} />
        ...
        <section className="py-20 px-4">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="text-xl text-muted-foreground mb-8 max-w-2xl mx-auto">
              From instant extraction to AI-powered analysis, discover why designcontracts.sh is the fastest way
          ...
          <div className="max-w-4xl mx-auto">
            <div className="text-center mb-12">
              ...
              <p className="text-muted-foreground">
                See why teams choose designcontracts.sh over manual extraction or limited tools
              </p>
            </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="text-left py-4 px-4 font-medium text-blue-600 dark:text-blue-400">designcontracts.sh</th>
                      ...
                      <div className="space-y-6">
                        ...
                        <div>
                          ...
                          <div className="bg-muted rounded-lg p-4 font-mono text-xs overflow-x-auto">
                            <pre>{`Framework Detection System:
...
Detection Strategy:
  → Class pattern matching using regex
  → Data attribute scanning
  → Component structure validation
  → Computed style fingerprinting`}</pre>
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div className="bg-muted rounded-lg p-4 font-mono text-xs overflow-x-auto">
                            <pre>{`Behavioral Pattern Detection:

Transitions:
  • Instant: transitionDuration = 0s
  • Smooth: transitionDuration greater than 0
  • Elastic: cubic-bezier or spring timing functions

Animations:
  • Pulse/Beat patterns
  • Spin/Rotate patterns
  • Bounce/Jump patterns
  • Fade/Opacity patterns
  • Slide/Translate patterns
...
State Change Detection:
  ...
  • Loading patterns: skeleton, shimmer, pulse, spinner
  • Focus trap detection
  • Drag behavior patterns
  • Scroll behavior patterns

Example - Ripple Detection:
  → Check for ripple child elements
  → Detect ripple animation names
  → Verify overflow:hidden for effect containment`}</pre>
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div className="bg-muted rounded-lg p-4 font-mono text-xs overflow-x-auto">
                            <pre>{`Cross-Element Pattern Validation:

Component Clustering Process:
  1. Generate unique signature for each element
  2. Find similar components using signature matching
  3. Group elements into clusters
...
Confidence Calculation:
  ...
  Boost factors:
  • 3+ instances: +0.15 (repetition increases confidence)
  • 5+ instances: +0.10 (strong pattern)
  • High consistency: +0.15 (based on similarity score)
  • 2+ variants: +0.10 (indicates intentional design)
  • 10+ instances: +0.05 (statistical significance)

  Maximum confidence: 1.0

Variant Detection:
  → Extract variant keys from each element
  → Group elements by variant type
  → Track variant types, instances, and counts
  → Common variants: size, color, state variations

Outlier Detection:
  → Compare each instance to cluster average
  → Flag instances with less than 30% similarity
  → Helps identify misclassified or inconsistent elements
...
51 Detected Component Types:
  ...
  toggle-group, tooltip, typography`}</pre>
                          </div>
                        </div>
                ...
                <div className="grid lg:grid-cols-2 gap-8">
                  <Card className="border-muted">
                    <CardContent className="p-6">
                      ...
                      <div className="bg-muted rounded-lg p-4 font-mono text-xs overflow-x-auto mb-4">
                        <pre className="text-foreground whitespace-pre-wrap">
                          <div className="space-y-1">
                            <div>// Intelligent context window utilization</div>
                            <div>ContextOptimizer System:</div>
                            <div>&nbsp;&nbsp;- MAX_TOKENS: 128,000 (GPT-4o limit)</div>
                            <div>&nbsp;&nbsp;- RESERVED_OUTPUT: 4,096 tokens</div>
                            <div className="mt-2">Core Methods:</div>
                            <div>&nbsp;&nbsp;- optimizePrompt(): Smart token management</div>
                            <div>&nbsp;&nbsp;- compressCss(): Semantic compression with token prioritization</div>
                            <div>&nbsp;&nbsp;- estimateTokens(): ~4 characters per token approximation</div>
                            <div>&nbsp;&nbsp;- enrichWithContext(): Context enhancement strategies</div>
                            <div className="mt-2">Features:</div>
                            <div>&nbsp;&nbsp;- Prioritizes design token selectors</div>
                            <div>&nbsp;&nbsp;- Removes comments and deduplicates rules</div>
                            <div>&nbsp;&nbsp;- Compresses to exact token limits</div>
                          </div>
                        </pre>
                      </div>
                      <ul className="text-sm space-y-1 text-muted-foreground">
                  ...
                  <Card className="border-muted">
                    <CardContent className="p-6">
                      ...
                      <div className="bg-muted rounded-lg p-4 text-xs mb-4">
                        <div className="space-y-2 text-muted-foreground">
                          <p><strong className="text-foreground">Few-shot learning</strong> with dynamic examples improves accuracy by 23%</p>
                          <p><strong className="text-foreground">Vector similarity search</strong> selects domain-relevant examples (similarity &gt; 0.7)</p>
                          <p><strong className="text-foreground">Compressed JSON format</strong> reduces output tokens by 45%</p>
                          <p><strong className="text-foreground">Example selection</strong> based on performance scoring and domain relevance</p>
                        </div>
                      </div>
                      <ul className="text-sm space-y-1 text-muted-foreground">
...
class designcontracts.shMCPServer implements MCPServer {
  private tools = [
    'scan_tokens',
    'get_layout_dna',
    'analyze_brand',
        ...
        <section className="py-16 px-4 bg-muted/30">
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-12">
              ...
              <p className="text-muted-foreground">
                See how developers integrate designcontracts.sh into their actual workflows
              </p>
            </div>
  ...
  sync-design-tokens:
    ...
    steps:
      ...
      - name: Extract Latest Design Tokens
        run: |
          curl -X POST "https://designcontracts.sh/api/v1/scan" \\
      ...
      - name: Create Pull Request
        ...
        with:
          ...
          body: |
            ...
            🤖 Generated by designcontracts.sh CI/CD integration`}</pre>
                      </div>
                      <ul className="text-sm space-y-1 text-muted-foreground">
