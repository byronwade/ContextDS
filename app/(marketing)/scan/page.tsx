    ...
    <div className="flex h-screen w-full flex-col overflow-hidden antialiased">
      ...
      {isLoading && (
        <>
          ...
          {progress && (
            <div className="absolute top-16 left-1/2 -translate-x-1/2 z-50 bg-background/95 backdrop-blur-sm border border-grep-2 rounded-lg shadow-lg px-3 sm:px-4 py-3 min-w-[280px] sm:min-w-[320px] max-w-[calc(100vw-2rem)]">
              <div className="flex items-center gap-2 sm:gap-3 mb-2">
                <div className="w-2 h-2 rounded-full bg-blue-500 animate-pulse shrink-0" />
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-foreground truncate">{progress.phase}</div>
                  <div className="text-xs text-grep-9 mt-0.5 truncate">{progress.message}</div>
                </div>
                <div className="text-xs font-mono text-grep-9 tabular-nums shrink-0">
                ...
                <div className="mt-2 space-y-1 border-t border-grep-2 pt-2">
                  {progress.details.slice(-3).map((detail, i) => (
                    <div key={i} className="text-xs text-grep-9 font-mono flex items-center gap-2">
                      <div className="w-1 h-1 rounded-full bg-grep-7 shrink-0" />
      ...
      <header className="flex min-h-[56px] sm:min-h-[64px] w-full shrink-0 flex-wrap items-center justify-between border-b border-grep-2 md:flex-nowrap">
          ...
          <div className="relative z-10 w-full flex-grow max-w-2xl">
            ...
            <div className="absolute left-2 sm:left-3 top-1/2 -translate-y-1/2 flex items-center gap-1.5 sm:gap-2 z-20">
              ...
              >
                ...
                <span className="capitalize hidden xs:inline">{searchMode}</span>
            ...
            {/* Right Controls */}
            <div className="absolute right-1.5 sm:right-2 top-1/2 flex -translate-y-1/2 items-center gap-1">
              {searchMode === "scan" && (
                <Button
                  onClick={handleSearch}
                  disabled={!searchQuery.trim()}
                  size="sm"
                  className="h-7 sm:h-7 px-2 xs:px-2.5 sm:px-3 bg-gradient-to-r from-emerald-500 to-teal-500 text-white text-[11px] xs:text-xs font-medium rounded hover:from-emerald-600 hover:to-teal-600 transition-all duration-200 shadow-none border-0"
                >
        ...
        <div className="flex min-h-[56px] sm:min-h-[64px] select-none items-center justify-end gap-1.5 sm:gap-2 pr-2 sm:pr-4 md:pr-6">
          ...
          {cacheTimestamp && !isRevalidating && (
            <div className="hidden md:flex items-center gap-2 px-2 py-1 rounded border border-grep-2 bg-grep-0">
          ...
          {isRevalidating && (
            <div className="hidden md:flex items-center gap-2 px-2 py-1 rounded border border-blue-200 dark:border-blue-900 bg-blue-50 dark:bg-blue-950/30">
          ...
          {displayResult && !isLoading && (
            <Button
              ...
              variant="ghost"
              size="sm"
              className="hidden md:inline-flex h-7 sm:h-8 px-2 sm:px-3 text-xs font-medium text-grep-9 hover:text-foreground"
              title="Force refresh scan"
            >
              <Loader2 className="h-3.5 w-3.5 sm:mr-1.5" />
